...
def dashboard():
    ...
    user_transactions = [
        t for t in TRANSACTIONS if t.get("from_user") == session["user"]
    ][-5:]

    return render_template(
        "dashboard.html", user=user_data, transactions=user_transactions
    )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if amount > user_data["balance"]:
                return render_template(
                    "transfer.html", error="Insufficient balance", user=user_data
                )
