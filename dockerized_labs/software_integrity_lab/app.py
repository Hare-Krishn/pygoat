...
def lab1():
    ...
    if token is None:
        ...
        response = make_response(
            render_template("lab1.html", message="Only Admins can see this page")
        )
        ...
        try:
            ...
            admin = pickle.loads(
                token
            )  # Intentionally vulnerable to pickle deserialization
            if admin.admin == 1:
                response = render_template(
                    "lab1.html", message="Welcome Admin, SECRETKEY:ADMIN123"
                )
...
def serve_file(filename):
    ...
    return send_from_directory(
        "static", filename, as_attachment=True, download_name=filename
    )
