...
def init_db():
    with app.app_context():
        ...
        if not Admin.query.filter_by(username=ADMIN_USERNAME).first():
            ph = PasswordHasher()
            admin = Admin(username=ADMIN_USERNAME, password=ph.hash(ADMIN_PASSWORD))
        ...
        if email:
            ...
            if email == "admin@pygoat.com":
                ...
                response = make_response(
                    render_template("lab1.html", otp_form=True, otp=otp)
                )
                response.set_cookie("email", email)
                return response
        ...
        try:
            ...
            if not user:
                ...
                return render_template(
                    "lab2.html", success=False, failure=True, error="Invalid username"
                )
            ...
            try:
                ...
                if fail_attempts == 5:
                    ...
                    db.session.commit()
                    return render_template(
                        "lab2.html", success=False, failure=True, is_locked=True
                    )
                user.fail_attempts = fail_attempts
                db.session.commit()
                # Vulnerable: Password-specific error message
                return render_template(
                    "lab2.html", success=False, failure=True, error="Invalid password"
                )
...
def lab3():
    if request.method == "GET":
        try:
            ...
            if session_id:
                try:
                    ...
                    if (
                        session_info.get("valid_until")
                        and datetime.fromisoformat(session_info["valid_until"])
                        > datetime.now()
                    ):
                        return render_template(
                            "lab3.html", username=session_info["username"], success=True
                        )
                except:
                    pass
        ...
        if username in USERS and USERS[username]["password"] == password_hash:
            ...
            response = make_response(
                render_template("lab3.html", success=True, username=username)
            )
