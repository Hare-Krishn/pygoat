...
def lab():
    ...
    if request.method == "POST":
        ...
        # Use ticket
            ...
            try:
                ...
                if (
                    sold_tickets < 60
                ):  # Vulnerability: Fixed number makes it predictable
                    error = f"Wait until all tickets are sold ({60-sold_tickets} tickets left)"
                else:
                    ticket = Ticket.query.filter_by(
                        ticket_code=ticket_code, user_id=user.id
                    ).first()
                    if ticket:
    ...
    return render_template(
        "lab.html", tickets=[t.ticket_code for t in user.tickets], error=error
    )
